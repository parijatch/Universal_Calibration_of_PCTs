...
get_Cauchy <- function(n){
  return(tan(pi*(runif(n)-1/2)))
}
...
mvt.effect.of.dimension.and.nu <- function(n=10^6,
  ...
  for (d in dims){
    ...
    for (nu in nus){
      cat(" ",nu)
      out = experiment.multivariate.t(n=n,d=d,nu=nu,sigma.half=A,to.plot = T);
...
#######
generate_psd_matrix <- function(d) {
...
#######
Cauchy.factor.model.experiment <- function(n=10^5,d=10,p=10, 
                                    w = rep(1,d)/d,
                                    A = matrix(rnorm(d*p),d,p),
                                    t = n^(0.5)*c(1:100)/100,
                                    to.plot=TRUE){
  
  A0 = A/outer(rowSums(abs(A)),rep(1,p))
  Z = matrix(get_Cauchy(n*p),n,p)
  Y = Z%*%t(A)
  X.pareto = Pareto.scale(Y);
  T.pareto = X.pareto %*% w;
  ...
  if (to.plot){
    plot(t,P.pareto,type="l",ylim=c(-0.5,1.2),cex=0.75,lwd=2,
         main = paste0("Multivariate Cauchy factor model: ",
                       " p=",p,
                       " d=",d,
                       " n=",n),
         xlab="1/alpha",ylab="combined p-value/alpha");
    ...
    Cauchy.limit = sum(abs(w%*%A0)) ## CHECK the formula!
      
    abline(h=Cauchy.limit,col="green",lty=4)
    legend("bottomleft",cex=0.5, legend=c("Pareto comb test","Cauchy comb test",
                                          "Cauchy limit (theoretical)"),
           col=c("black","red","green"),lwd=c(2,2,1),lty=c(1,1,4));
    abline(h=c(0,1),lty=3,col="blue")
  }
  return(list("t"=t, "P.pareto"=P.pareto,"P.cauchy"=P.cauchy,
